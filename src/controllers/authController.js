...
// /**
//  * ✅ NEW CONTROLLER (ONLY ADDITION)
//  * Returns ALL users under the current user in the reportsTo tree (multi-level).
//  * Admin -> BranchManagers + TeamLeads + Employees
//  * BranchManager -> TeamLeads + Employees
//  * TeamLead -> Employees
//  */
...
//     // Breadth-first search through reportsTo tree
//     const queue = [myId];
...
// --- REGISTER USER CONTROLLER ---
// ✅ MODIFIED: Admin can create HR or BranchManager. BM/HR create TeamLead. TL create Employee.
// Also supports: branch + salary fields (if your User model has them)
exports.registerUser = async (req, res) => {
  ...
  try {
    ...
    if (creatorRole === 'Admin') {
      // Admin can create HR or BranchManager
      ...
      return res.status(403).json({ msg: "You are not authorized to create users." });
    }
    ...
    user = new User({
      ...
      // optional fields (only if your User schema includes them)
